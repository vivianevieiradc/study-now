...
function PlanoView({ plan, setPlan, disciplines, discById }) {
  ...
  return <div className="xl:-mx-5">
    ...
    <div className={"grid gap-4 " + (showMonth ? "" : "xl:grid-cols-[minmax(270px,0.85fr)_minmax(0,1.7fr)]")}>
      ...
      <Card className={"!p-5 " + (showMonth ? "min-h-[520px] xl:col-span-full" : "min-h-[620px]")}>
        {showMonth ? <div>
          ...
          <div className="overflow-x-auto"><div className="min-w-[760px]"><div className="grid grid-cols-7 mb-2">{calendarDays.map((day) => <span key={day} className="text-[10px] font-bold text-center" style={{ color: C.muted }}>{day}</span>)}</div><div className="grid grid-cols-7">{cells.map((date) => { const dateISO = localISO(date); const current = date.getMonth() === monthDate.getMonth(); const today = dateISO === todayISO(); const selected = dateISO === selectedDateISO; const items = current ? plan.filter((p) => p.date === dateISO) : []; return <div key={dateISO} className="h-[104px] overflow-hidden p-1.5" style={{ border: "1px solid " + C.line, background: selected ? C.goldSoft : C.surface2, opacity: current ? 1 : .45 }}><button onClick={() => { selectDate(date); setShowMonth(false); }} className="flex items-center justify-between w-full mb-2"><b className="text-xs" style={{ color: today ? C.gold : C.ink }}>{date.getDate()}</b>{today && <span className="text-[10px] font-bold" style={{ color: C.gold }}>Hoje</span>}</button><div className="space-y-1">{items.slice(0, 2).map((it) => { const d = discById[it.disciplineId]; const topic = it.topicId ? d?.topics?.find((t) => t.id === it.topicId)?.name : null; return <button key={it.id} onClick={() => { selectDate(date); setShowMonth(false); }} title={[d?.name, topic].filter(Boolean).join(" · ")} className="w-full text-left rounded-md px-2 py-1.5" style={{ background: d?.color || C.chip, color: "#fff" }}><span className="block text-[10px] leading-tight font-semibold whitespace-nowrap overflow-hidden text-ellipsis">{disciplineShort(d?.name || "Disciplina")}</span>{topic && <span className="block text-[9px] leading-tight opacity-90 whitespace-nowrap overflow-hidden text-ellipsis">{topic}</span>}<span className="block text-[9px] mt-0.5 opacity-80">{fmtMin(it.minutes)}</span></button>; })}{items.length > 2 && <div className="text-[10px] text-center" style={{ color: C.muted }}>+{items.length - 2} sessões</div>}</div></div>; })}</div></div></div>

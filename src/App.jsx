...
/* ============================================================
   EDITAL VERTICALIZADO — Escriturário / Agente de Tecnologia
   BB (Edital 2022/001, CESGRANRIO) · peso=pontos · q=nº questões
   hits = incidência somada (provas 2021 DF-TI + 2022 Micro 158-TI)
   ============================================================ */
const SEED = [
  { name: "Tecnologia da Informação", block: "Específicos", peso: 52.5, q: 35, topics: [
      { name: "Banco de Dados (SQL, NoSQL, modelagem ER, normalização, DW, SGBD, MongoDB, PostgreSQL)", hits: 20 },
      { name: "Estrutura de dados e algoritmos (busca, ordenação, pilha, fila, árvore, lista)", hits: 12 },
      { name: "Python 3.9 aplicado a dados (Pandas, NumPy, SciPy, Matplotlib, Scikit-learn)", hits: 7 },
      { name: "Java (SE 11 / EE 8)", hits: 6 },
      { name: "TypeScript 4.0", hits: 5 },
      { name: "Desenvolvimento Mobile (Java/Kotlin, Swift, React Native, Android, iOS)", hits: 8 },
      { name: "Aprendizagem de máquina e Processamento de Linguagem Natural", hits: 5 },
      { name: "Big Data (fundamentos, preparação e apresentação de dados)", hits: 5 },
      { name: "Ansible (playbooks / YAML)", hits: 2 },
  ]},
  ...
  { name: "Matemática", block: "Básicos", peso: 7.5, q: 5, topics: [
      { name: "Razão e proporção, regra de três, porcentagem", hits: 3 },
      { name: "Funções (polinomiais, exponenciais, logarítmicas)", hits: 2 },
      { name: "Progressões (PA e PG) e sequências", hits: 1 },
      { name: "Lógica proposicional", hits: 1 },
      { name: "Sistemas lineares", hits: 1 },
      { name: "Problemas de contagem", hits: 1 },
      { name: "Conjuntos", hits: 0 },
      { name: "Matrizes e determinantes", hits: 0 },
  ]},
  { name: "Probabilidade e Estatística", block: "Específicos", peso: 7.5, q: 5, topics: [
      { name: "Probabilidade (condicional, total, Teorema de Bayes)", hits: 5 },
      { name: "Variância, covariância e dispersão", hits: 3 },
      { name: "Distribuição binomial e normal", hits: 2 },
      { name: "Amostragem e inferência estatística", hits: 1 },
      { name: "Medidas de tendência central", hits: 1 },
      { name: "Representação tabular e gráfica", hits: 0 },
      { name: "Correlação linear simples", hits: 0 },
  ]},
  { name: "Conhecimentos Bancários", block: "Específicos", peso: 7.5, q: 5, topics: [
      { name: "SFN, política monetária e Selic/Copom", hits: 3 },
      { name: "Sigilo bancário (LC 105/2001)", hits: 2 },
      { name: "Mercado de câmbio e taxas de câmbio", hits: 2 },
      { name: "Orçamento público e dívida pública", hits: 1 },
      { name: "Prevenção à lavagem de dinheiro (Lei 9.613, Carta-Circular 4.001)", hits: 1 },
      { name: "Responsabilidade socioambiental e ASG", hits: 1 },
      { name: "Produtos bancários e varejo bancário", hits: 1 },
      { name: "Garantias do SFN, LGPD, ética, segurança cibernética", hits: 0 },
  ]},
  { name: "Atualidades do Mercado Financeiro", block: "Básicos", peso: 5, q: 5, topics: [
      { name: "Fintechs, bancos digitais, startups e big techs", hits: 2 },
      { name: "Funções da moeda", hits: 2 },
      { name: "Open banking", hits: 1 },
      { name: "PIX e arranjos de pagamento", hits: 1 },
      { name: "Shadow banking (bancos-sombra)", hits: 1 },
      { name: "Blockchain, bitcoin e criptomoedas", hits: 1 },
      { name: "Marketplace", hits: 1 },
      { name: "Sistemas do BCB (ex.: Registrato) e transformação digital", hits: 1 },
  ]},
...
function seedSims(disc) {
  const mk = () => disc.map((d) => ({ disciplineId: d.id, right: 0, total: d.q }));
  return [
    { id: uid(), name: "Prova 158-TI (2023)", date: "2023-04-23", rows: mk() },
    { id: uid(), name: "Prova DF-TI (2021)", date: "2021-07-25", rows: mk() },
  ];
}
...
function StudyApp({ onLogout }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      let d = await store.get("disc", null);
      if (!d) {
        d = SEED.map((s, i) => ({ id: uid(), name: s.name, block: s.block, peso: s.peso, q: s.q, color: DISC_COLORS[i % DISC_COLORS.length], topics: s.topics.map((t) => ({ id: uid(), name: t.name, hits: t.hits, studied: false })) }));
        await store.set("disc", d);
      }
      setDisciplines(d);
      setSessions(await store.get("sess", []));
      setReviews(await store.get("rev", []));
      setPlan(await store.get("plan", []));
      let sm = await store.get("sim", null); if (!sm) { sm = seedSims(d); await store.set("sim", sm); } setSimulados(sm);
      setGoals(await store.get("goals", { hours: 20, questions: 200 }));
      setPomo(await store.get("pomo", { focus: 25, short: 5, long: 15, rounds: 4, autostart: true }));
      let cy = await store.get("cycle", null);
      if (!cy) { cy = { mode: "auto", blocks: autoCycle(d) }; await store.set("cycle", cy); }
      setCycle(cy);
      setLoading(false);
    })();
  }, []);
  ...
  useEffect(() => { if (!loading) store.set("disc", disciplines); }, [disciplines, loading]);
  useEffect(() => { if (!loading) store.set("sess", sessions); }, [sessions, loading]);
  useEffect(() => { if (!loading) store.set("rev", reviews); }, [reviews, loading]);
  useEffect(() => { if (!loading) store.set("cycle", cycle); }, [cycle, loading]);
  useEffect(() => { if (!loading) store.set("plan", plan); }, [plan, loading]);
  useEffect(() => { if (!loading) store.set("goals", goals); }, [goals, loading]);
  useEffect(() => { if (!loading) store.set("sim", simulados); }, [simulados, loading]);
        ...
        <div className="flex">
          ...
          <main className="flex-1 min-w-0">
            <header className="md:hidden flex items-center gap-3 px-4 py-3 border-b sticky top-0 z-30" style={{ background: C.surface, borderColor: C.line }}>
              ...
              <span className="font-bold flex-1">Estudei</span>
...
function Brand() { const C = useC(); return (<div className="flex items-center gap-2.5 px-5 py-5"><div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: BRAND }}><BookOpen size={18} color="#F5B301" /></div><div><div className="font-extrabold text-lg leading-none" style={{ color: C.ink }}>Estudei</div><div className="text-[11px]" style={{ color: C.muted }}>BB · Agente de Tecnologia</div></div></div>); }
...
function RaioXView({ disciplines }) {
  ...
  return (
    <div>
      <PageTitle sub="Baseado no edital 2022 (pesos oficiais) cruzado com as provas Cesgranrio 2021 DF-TI e 2022 Micro 158-TI.">Raio-X da prova</PageTitle>
      <Card className="mb-4">
...
function RevisoesView({ reviews, setReviews, markReviewDone, discById, disciplines }) {
  ...
  const Group = ({ title, items, color }) => items.length ? <div className="mb-5"><h3 className="text-sm font-bold mb-2 flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: color }} />{title} <span style={{ color: C.muted }}>({items.length})</span></h3><div className="space-y-2">{items.map((r) => (<Card key={r.id} className="!p-3 flex items-center gap-3"><button onClick={() => markReviewDone(r.id)}><Circle size={22} color={C.muted} /></button><div className="flex-1 min-w-0"><div className="font-medium text-sm truncate">{r.label}</div><div className="text-xs" style={{ color: C.muted }}>{discById[r.disciplineId]?.name} · intervalo {REVIEW_INTERVALS[r.intervalIdx]}d</div></div><input type="date" value={r.due} onChange={(e) => editDate(r.id, e.target.value)} className="px-2 py-1 rounded-lg text-xs" style={inputStyle(C)} /></Card>))}</div></div> : null;
  return <div>
...
/* ============================ EDITAL ============================ */
function EditalView({ disciplines, sessions }) {
  const C = useC();
  ...
  return <div>
    <PageTitle sub="Edital 2022 do Agente de Tecnologia, com peso, nº de questões e incidência histórica por tópico. Marque o que já estudou.">Edital verticalizado</PageTitle>
        ...
        {disciplines.filter((d) => d.block === block).map((d) => { const done = d.topics.filter((t) => t.studied).length;
          return <Card key={d.id} className="!p-4"><div className="flex items-center gap-2 mb-3"><span className="w-1.5 h-6 rounded-full" style={{ background: d.color }} /><span className="font-bold flex-1">{d.name}</span><span className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: C.goldSoft, color: C.ink }}>{d.peso} pts · {d.q}q</span><span className="text-xs" style={{ color: C.muted }}>{done}/{d.topics.length}</span></div><div className="space-y-1.5">
            {[...d.topics].sort((a, b) => b.hits - a.hits).map((t) => { const p = perf[t.id]; const tot = p ? p.r + p.w : 0; const acc = tot ? Math.round((p.r / tot) * 100) : null; const weak = acc !== null && acc < 60;
              return <div key={t.id} className="flex items-center gap-2 text-sm py-1">{t.studied ? <CheckCircle2 size={15} color={C.green} className="shrink-0" /> : <Circle size={15} color={C.line} className="shrink-0" />}<span className="flex-1 min-w-0" style={{ color: t.studied ? C.ink : C.muted }}>{t.name}</span>{t.hits >= 8 && <span className="text-[10px] px-1.5 py-0.5 rounded-full shrink-0" style={{ background: C.redSoft, color: C.red }}>cai muito</span>}{t.hits >= 4 && t.hits < 8 && <span className="text-[10px] px-1.5 py-0.5 rounded-full shrink-0" style={{ background: C.goldSoft, color: C.ink }}>cai bastante</span>}{p && <span className="text-xs shrink-0" style={{ color: C.muted }}>{fmtMin(p.min)}</span>}{acc !== null && <span className="text-xs font-semibold px-2 py-0.5 rounded-full shrink-0" style={{ background: weak ? C.redSoft : C.greenSoft, color: weak ? C.red : C.green }}>{acc}%{weak && " · foco"}</span>}</div>;
            })}
          </div></Card>;
        })}
      </div></div>
    ))}
    <p className="text-xs" style={{ color: C.muted }}>Incidência estimada a partir das provas Cesgranrio 2021 (DF-TI) e 2022 (Micro 158-TI). Guia de prioridade — a próxima prova pode variar.</p>
  </div>;
}
...
function SimModal({ disciplines, onClose, onSave, initial }) {
  ...
  return <Modal open title={initial ? "Editar simulado" : "Registrar simulado"} onClose={onClose}>
    <Field label="Nome"><input value={name} onChange={(e) => setName(e.target.value)} className={inputCls} style={inputStyle(C)} placeholder="Ex.: Prova BB 2022 / Simulado 01" /></Field>
...
export default function App() {
  ...
  return <StudyApp onLogout={() => supabase.auth.signOut()} />;
}
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: P.bg, fontFamily: "'Inter',ui-sans-serif,system-ui,sans-serif", padding: 16 }}>
      <form onSubmit={submit} style={{ width: "100%", maxWidth: 380, background: P.surface, border: `1px solid ${P.line}`, borderRadius: 20, padding: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
          ...
          <div><div style={{ fontWeight: 800, fontSize: 20, color: P.ink }}>Estudei</div><div style={{ fontSize: 11, color: P.muted }}>BB · Agente de Tecnologia</div></div>
        </div>
